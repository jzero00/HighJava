...
public class DOMParsing {
	...
	try {
	    ...
	    //속성접근방법1 : 엘리먼트 객체의 getAttribute()메소드 이용
	    String isbn = bookElement.getAttribute("isbn");
	    System.out.println("bookElement.getAttribute(\"isbn\") = > " + isbn);
	    
	    //속성 접근방법2: 노드객체의 getAttributes()메소드 이용 (속성노드 접근)
	    NamedNodeMap nodeMap = firstBookNode.getAttributes();
	    System.out.println("노드 객체의 getAttribute()메소드 이용 : " + nodeMap.getNamedItem("isbn").getNodeValue());
	    ...
	    //( getChildrens()보다는, getElementByTagName()을 이용하는게 좋다.)
