...
public class T01_FileTest {
	...
	/*
	 * 디렉토리 (폴더) 만들기 1. mkdir() → File객체의 경로중 마지막 위치의 디렉토리를 만든다. → 중간의 경로가 모두 미리
	 * 만들어져 있어야 한다.
	 * 
	 * 2. mkdirs() → 중간의 경로가 없으면 중간의 경로도 새롭게 만든 후 마지막 위치의 디렉토리를 만들어준다. → 위 두 메소드 모두
	 * 만들기를 성공하면 true, 실패하면 false 반환
	 */
