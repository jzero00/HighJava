package b_GenericEnumTest;

public class T02_ArgsTest {
/*
 * 가변형 인수 → 메소드의 매개변수의 개수가 실행될때마다 다를 때 사용한다.
 * 	- 가변형 인구는 메소드 안에서 배열로 처리된다.
 * 	- 가변형 인수는 한가지 자료형만 사용할 수 있다.
 */
	
	//배열을 이용한 메소드
	//매개변수로 받은 정수들의 합계를 구하는 메소드 (이 정수들의 개수는 상황에 따라 다르다)
	...
	public int sumArg(int...data) {
		int sum = 0;
		for(int i = 0; i < data.length; i++) {	//가변형 인수를 사용할때 "..."를 사용
			sum += data[i];
		}
		return sum;
	}
